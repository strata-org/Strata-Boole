...
Tests the validate-candidate workflow: when cvc5 returns `unknown (some m)` for
nat-using VCs, the candidate model is validated in pure Lean and promoted to
`❌ fail` with a concrete integer counterexample.

These cover the SAT direction from the proposal's benchmark table.
-/
...
Obligation: assert_1_887
Property: assert
Result: ❌ fail

Obligation: test_false_lt_ensures_0_846
Property: assert
Result: ❌ fail
-/
#guard_msgs in
